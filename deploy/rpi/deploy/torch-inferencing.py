...
model_resnet.eval()
with torch.no_grad():
    input_test_rotor = torch.unsqueeze(
        input_test_rotor,
        0)  # adds another dimension (N C H W)
    print(input_test_rotor.size())
    ...
    input_test_misal = torch.unsqueeze(
        input_test_misal,
        0)  # adds another dimension (N C H W)
    ...
    input_test_healthy = torch.unsqueeze(
        input_test_healthy,
        0)  # adds another dimension (N C H W)
    input_test_healthy = input_test_healthy.to(
        device)  # send data to GPU or CPU
